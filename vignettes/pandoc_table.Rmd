...
Core functionality of `pander` is centered around `pandoc.table` which is aimed at rendering tables in `markdown`. In case of 2D tables, `pander` calls `pandoc.table` internally, thus in such cases `pander` and `pandoc.table` support the same argument and in this vignette will be used iterchangingly. `pandoc.table` has big variety of options ([highlighting](#highlighting-cells), [styles](#table-styles), etc.) and this vignette aims to give a more details overview of most common options. `pander` comes with a variety of globally adjustable options, which have an effect on the result of your reports. You can query and update these options with the `panderOptions` function.

## Table styles

Since `pander` aims at rendering R objects into [Pandoc](http://pandoc.org/)'s markdown all four (`multiline`, `simple`, `grid`, `rmarkdown`) of  [Pandoc](http://johnmacfarlane.net/pandoc/README.html#tables)'s formats are supported. General advices is to stick with default `multiline` style, but if there is a need to change it either specify `style` argument when calling `pander/pandoc.table` or change default `style` using `panderOptions`.

### multiline tables

`multiline` tables allow headers and table rows to span multiple lines of text (but cells that span multiple columns or rows of the table are not supported). Also note, that, by default, line breaks are removed from cells for simplicity, so multiline cells are typically the result of [splitting large cells](#table-and-cell-width) or setting `keep.line.breaks` to `TRUE`:
...
`rmarkdown` or pipe table format is often used directly with `knitr`, since it was supportered by the first versions of the `markdown` package. It is similar to `simple` table, meaning that multiline cells are not supported. The beginning and ending pipe characters are optional, but pipes are required between all columns:
...
`pander` allows users to control cell alignment (`left`, `right` or `center/centre`) in a table directly by setting the `justify` parameter when calling `pander/pandoc.table`. Note that it is possible to specify alighnment for each column separately by supplying a vector to justify:
...
Another possible way to define alignment is by using a permanent option `table.alignment.default/table.alignment.rownames` in `panderOptions` (prefered way) or by using `set.alignment` function (legacy way of setting alignment for next table or permanently) which support setting alignment separately for cells and rownames:
...
`pander/pandoc.table` is able to deal with the problem of  wide tables. Ever had an issue in `LaTeX` or `MS Word` when tried to print a correlation matrix of 40 variables? This problem is carefully addressed with `split.table` parameter:
...
In some cases it is also useful to split too long words like with hyphens, and `pander` uses `koRpus` functionality for that. Just specify `use.hyphening` argument and have `koRpus` installed:
...
`round` and `digits` param can be a vector specifying values for each column (has to be the same length as number of columns). Values for non-numeric columns will be disregarded.
