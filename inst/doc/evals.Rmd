...
To make execution and debugging easier to understand, `evals` provides logging with the `log` parameter. Logging in `evals` relies on the [`futile.logger`](cran.r-project.org/web/packages/futile.logger/futile.logger.pdf) package,  which provides a logging API similar to `log4j`. Basic example:
